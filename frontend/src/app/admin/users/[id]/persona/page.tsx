...
export default function AdminPersonaPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApi<Persona[]>(useCallback(() => adminApi.personaList(userId), [userId]));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPersona) {
        await adminApi.personaUpdate(userId, editingPersona.id, payload);
      } else {
        await adminApi.personaCreate(userId, payload);
      }
      setDialogOpen(false);
      refetch();
    } catch {
  ...
  const handleDelete = async (id: number) => {
    setDeleting(id);
    try {
      await adminApi.personaDelete(userId, id);
      refetch();
    } catch {
      // Error handled by API client
    } finally {
  ...
  const handleActivate = async (id: number) => {
    setActivating(id);
    try {
      await adminApi.personaActivate(userId, id);
      refetch();
    } catch {
      // Error handled by API client
    } finally {
      ...
      <CardContent className="p-5 space-y-3">
        ...
        {(persona.personality_traits ?? []).length > 0 && (
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-medium">パーソナリティ</p>
            <div className="flex flex-wrap gap-1">
              {persona.personality_traits.map((trait) => (
                <Badge key={trait} variant="outline" className="border-purple-500/30 text-purple-400 text-[10px]">
                  {trait}
                </Badge>
              ))}
            </div>
        ...
        {persona.target_audience && (
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-medium">ターゲット</p>
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              {persona.expertise_areas.map((area) => (
                <Badge key={area} variant="outline" className="border-cyan-500/30 text-cyan-400 text-[10px]">
                  {area}
                </Badge>
              ))}
            </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-foreground">ペルソナ管理</h2>
        ...
        <DialogContent className="bg-card border-border sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input placeholder="例: テック系インフルエンサー" value={formData.name} onChange={(e) => updateField("name", e.target.value)} className="bg-muted border-border text-foreground" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">説明</Label>
              <Textarea placeholder="このペルソナの概要を記述してください" value={formData.description} onChange={(e) => updateField("description", e.target.value)} rows={2} className="bg-muted border-border text-foreground resize-none" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">パーソナリティ特性 (カンマ区切り)</Label>
              <Input placeholder="例: 知的好奇心旺盛, ポジティブ, 分析的" value={formData.personality_traits} onChange={(e) => updateField("personality_traits", e.target.value)} className="bg-muted border-border text-foreground" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">バックグラウンドストーリー</Label>
              <Textarea placeholder="このペルソナの背景や経歴を記述してください" value={formData.background_story} onChange={(e) => updateField("background_story", e.target.value)} rows={3} className="bg-muted border-border text-foreground resize-none" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">ターゲットオーディエンス</Label>
              <Input placeholder="例: 20-30代のエンジニア" value={formData.target_audience} onChange={(e) => updateField("target_audience", e.target.value)} className="bg-muted border-border text-foreground" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">専門分野 (カンマ区切り)</Label>
              <Input placeholder="例: AI, Web開発, スタートアップ" value={formData.expertise_areas} onChange={(e) => updateField("expertise_areas", e.target.value)} className="bg-muted border-border text-foreground" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-foreground/80">コミュニケーションスタイル</Label>
                <Select value={formData.communication_style} onValueChange={(value) => updateField("communication_style", value)}>
                  <SelectTrigger className="w-full bg-muted border-border text-foreground"><SelectValue placeholder="スタイルを選択" /></SelectTrigger>
                  <SelectContent className="bg-muted border-border">
                    {COMMUNICATION_STYLES.map((style) => (<SelectItem key={style.value} value={style.value} className="text-foreground focus:bg-muted focus:text-foreground">{style.label}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={formData.tone} onValueChange={(value) => updateField("tone", value)}>
                  <SelectTrigger className="w-full bg-muted border-border text-foreground"><SelectValue placeholder="トーンを選択" /></SelectTrigger>
                  <SelectContent className="bg-muted border-border">
                    {TONES.map((tone) => (<SelectItem key={tone.value} value={tone.value} className="text-foreground focus:bg-muted focus:text-foreground">{tone.label}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">言語パターン (カンマ区切り)</Label>
              <Input placeholder="例: 〜だよね, ちなみに, 実は" value={formData.language_patterns} onChange={(e) => updateField("language_patterns", e.target.value)} className="bg-muted border-border text-foreground" />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/80">投稿例 (1行につき1投稿)</Label>
              <Textarea placeholder={"例:\n今日もAIの進化が止まらない！特に...\nスタートアップで学んだ3つのこと..."} value={formData.example_posts} onChange={(e) => updateField("example_posts", e.target.value)} rows={4} className="bg-muted border-border text-foreground resize-none" />
            </div>
          </div>

          <DialogFooter>
            <Button variant="ghost" onClick={() => setDialogOpen(false)} className="text-muted-foreground">キャンセル</Button>
            <Button onClick={handleSave} disabled={saving || !formData.name.trim()} className="gap-2">
