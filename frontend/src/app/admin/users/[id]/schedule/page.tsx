...
export default function AdminSchedulePage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApi<Schedule[]>(useCallback(() => adminApi.scheduleList(userId), [userId]));
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSchedule) {
        await adminApi.scheduleUpdate(userId, editingSchedule.id, formData);
      } else {
        await adminApi.scheduleCreate(userId, formData);
      }
      setDialogOpen(false);
      refetch();
    } catch {
  ...
  const handleDelete = async (id: number) => {
    setDeleting(id);
    try {
      await adminApi.scheduleDelete(userId, id);
      refetch();
    } catch {
      // Error handled
    } finally {
  ...
  const handleToggle = async (id: number) => {
    try {
      await adminApi.scheduleToggle(userId, id);
      refetch();
    } catch {
      // Error handled
    }
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      {schedule.scheduled_at && (
                        <span>予定: {formatDate(schedule.scheduled_at)}</span>
                      )}
      ...
      {/* Upcoming timeline */}
      {(schedules ?? []).filter((s) => s.is_active && s.scheduled_at).length > 0 && (
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-foreground text-base">今後の予定</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-muted border-border">
                  <SelectItem value="once" className="text-foreground focus:bg-muted">一回のみ</SelectItem>
                  <SelectItem value="recurring" className="text-foreground focus:bg-muted">繰り返し</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-muted border-border">
                  <SelectItem value="original" className="text-foreground focus:bg-muted">オリジナル</SelectItem>
                  <SelectItem value="ai_generated" className="text-foreground focus:bg-muted">AI生成</SelectItem>
                  <SelectItem value="template" className="text-foreground focus:bg-muted">テンプレート</SelectItem>
                </SelectContent>
              </Select>
            </div>
