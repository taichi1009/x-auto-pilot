...
export default function AdminFollowingPage() {
  ...
  } = useApi<FollowTarget[]>(useCallback(() => adminApi.followList(userId), [userId]));

  const {
    data: stats,
    loading: statsLoading,
    refetch: refetchStats,
  } = useApi<FollowStats>(useCallback(() => adminApi.followStats(userId), [userId]));

  const handleDiscover = async () => {
    if (!keyword.trim()) return;
    setDiscovering(true);
    try {
      await adminApi.followDiscover(userId, keyword.trim());
      refetchTargets();
    } catch {
      // Error handled
    } finally {
  ...
  const handleExecute = async (id: number) => {
    setExecuting(id);
    try {
      await adminApi.followExecute(userId, id);
      refetchTargets();
      refetchStats();
    } catch {
      // Error handled
  ...
  const handleBatchFollow = async () => {
    ...
    for (const target of pending) {
      try {
        await adminApi.followExecute(userId, target.id);
      } catch {
        // Continue with next
      }
    }
