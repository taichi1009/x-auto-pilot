...
class PersonaService:
    ...
    def update_persona(self, persona_id: int, data: dict) -> Persona:
    ...
    def get_active_persona(self) -> Optional[Persona]:
        return (
            self.db.query(Persona).filter(Persona.is_active == True).first()
        )

    def activate_persona(self, persona_id: int) -> Persona:
        # Deactivate all personas
        self.db.query(Persona).update({"is_active": False})
